...
public class addComment extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            int hotelID = Integer.parseInt(request.getSession().getAttribute("hotelID").toString());
            out.print("ggg");
            System.out.println("request.getSession().getAttribute(\"clientID\").toString()");
            int clientID = Integer.parseInt(request.getSession().getAttribute("clientID").toString());
            out.print("g555gg");
            out.print(comment + " ," + rate + " ," + hotelID);
            HotelDB hotelDB = new HotelDB();
            hotelDB.addAReview(hotelID, clientID, rate, comment);
            out.print("done");
